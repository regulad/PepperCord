...
class Remind(Cog):
    ...
    async def remind(
        self, ctx: CustomContext, time: TimedeltaShorthand, *, message: str = "$this"
    ) -> None:
        """
        ...
        await ctx.send(
            f"I will remind you in {time}. Leave your DMs open.", ephemeral=True
        )
    ...
    async def remind_online(
        self, ctx: CustomContext, *, message: str = "$this"
    ) -> None:
        """
        ...
        await ctx["author_document"].update_db(
            {
                "$push": {
                    "remind_online": {"message": message, "channel": ctx.channel.id}
                }
            }
        )
        await ctx.send(
            f"I will remind you when you come online next. Leave your DMs open.",
            ephemeral=True,
        )
